...
M.filetype_attach = setmetatable({
    ...
    -- 		"BufWritePre",
    -- 		{ pattern = "*.go", command = ":lua vim.lsp.buf.format()", group = id }
    -- 	)
    -- end,
}, {
    __index = function()
...
M.custom_attach = function(client, bufnr)
    ...
    if client.server_capabilities.document_formatting then
        local id = vim.api.nvim_create_augroup("lsp_formatting", { clear = true })
        vim.api.nvim_create_autocmd(
            "BufWritePre",
            { pattern = "<buffer>", command = "lua vim.lsp.buf.format()", group = id }
        )
    end
